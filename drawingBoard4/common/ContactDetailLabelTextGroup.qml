...
GridView {
    ...
    function doFilter()
    {
        if(filterName === StringConstants.model_contactLabeLAndText)
        {
            theListView.model = contactDetailDelegateModel.parts.newPatientContactLabelAndText
        }
        if(filterName === StringConstants.model_ContactDetailLabel)
        {
            theListView.model = contactDetailDelegateModel.parts.newPatientContactDetailLabel
        }
        if(filterName === StringConstants.model_ContactDetailTxt)
        {
            theListView.model = contactDetailDelegateModel.parts.newPatientContactDetailText
        }
    }
    ContactDetailDelegateModel{
    ...
    Component.onCompleted:{
        parts.newPatientContactDetailLabel.filterOnGroup = StringConstants.model_ContactDetailLabel;
        parts.newPatientContactDetailText.filterOnGroup = StringConstants.model_ContactDetailTxt;
        parts.newPatientContactLabelAndText.filterOnGroup = StringConstants.model_contactLabeLAndText;
        //doFilter()
    }
}
